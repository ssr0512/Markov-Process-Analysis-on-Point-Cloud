...
def parse_args():
    ...
    parser.add_argument('--model', default='repsurf.repsurf_ssg_umb',
    ...
    # Training
    parser.add_argument('--batch_size', type=int, default=1, help='batch size in training [default: 64]')
    ...
    parser.add_argument('--epoch', default=300, type=int, help='number of epoch in training [default: 500]')
    ...
    parser.add_argument('--init', type=str, default=None, help='initializer for model [kaiming, xavier, ZerO]')

    # Evaluation
    parser.add_argument('--min_val', type=int, default=10, help='Min val epoch [default: 100]')
...
def test(model, loader, num_class=15, num_point=1024, num_votes=3, total_num=1):
    vote_correct = 0
    sing_correct = 0
    ...
    for j, data in enumerate(loader):
        ...
        for cat in np.unique(target.cpu()):
            classacc = vote_pred_choice[target == cat].eq(target[target == cat].long().data).cpu().sum()
    ...
    return sing_acc, vote_acc, class_acc
...
def main(args):
    ...
    shutil.copy('/home/XXX/Revisit/classification/models/repsurf/repsurf_ssg_umb.py', str(experiment_dir))
    shutil.copy('/home/XXX/Revisit/classification/modules/repsurface_utils.py', str(experiment_dir))
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        if epoch >= args.min_val:
            with torch.no_grad():
                sing_acc, vote_acc, class_acc = test(classifier.eval(), testDataLoader, num_point=args.num_point,
                ...
                log_string('Best Class Accuracy: %.2f' % (best_class_acc * 100))
                if vote_acc >= best_vote_acc:
