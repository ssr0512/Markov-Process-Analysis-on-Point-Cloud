...
class NetVLADBase(nn.Module):
    ...
    def forward(self, x):
        ...
        vlad = vlad.contiguous().view((-1, self.cluster_size * self.feature_size))   # B x (C*K)
        return vlad
...
class SpatialPyramidNetVLAD(nn.Module):
    def __init__(self, feature_size, max_samples, cluster_size, output_dim,
        ...
        # # max_samples[4] = 64
        # self.vlad4 = NetVLADBase(feature_size[4], max_samples[4], cluster_size[4], output_dim[4], gating, add_batch_norm)
        # # max_samples[5] = 32
        # self.vlad5 = NetVLADBase(feature_size[5], max_samples[5], cluster_size[5], output_dim[5], gating, add_batch_norm)
        
        sum_cluster_size = cluster_size[0] * output_dim[0] #+ cluster_size[1] * output_dim[1]\
                           # + cluster_size[2] * output_dim[2] + cluster_size[3] * output_dim[3]\
                           # + cluster_size[4] * output_dim[4]+ cluster_size[5] * output_dim[5]

        self.hidden_weights = nn.Parameter(torch.randn(sum_cluster_size, output_dim[0])* 1 / math.sqrt(feature_size[0]))
    ...
    # def forward(self, f0, f1, f2, f3, f4, f5):
        ...
        # v4 = self.vlad4(f4)
        # v5 = self.vlad5(f5)
        # vlad = torch.cat((v0, v1, v2, v3, v4, v5), dim=-1)
        vlad = torch.matmul(v0, self.hidden_weights)      # B x (1024*64) X (1024*64) x 256 -> B x 256
