...
from modules.pointnet2_utils import index_points
...
class OperatorND(nn.Module):
    ...
    def forward(self, neighbor_index: "(bs, vertice_num, neighbor_index)",
        ...
        """
        # print(self.weights.shape, self.bias.shape, self.displacement.shape)
        bs, vertice_num, neighbor_num = neighbor_index.size()
        neighbor_displacement = get_neighbor_displacement(
            vertices, neighbor_index)
        theta = neighbor_displacement @ self.displacement
        ...
        # print(activation_support.shape)
        # (bs, vertice_num, neighbor_num, support_num * out_channel)
        ...
        # print(feature_fuse.shape)
        # (bs, vertice_num, out_channel)
        return feature_fuse

class Pooling(nn.Module):
    def __init__(self, pooling_rate: int = 2, neighbor_num: int = 8, cuda=False):
        super().__init__()
        self.pooling_rate = pooling_rate
        self.neighbor_num = neighbor_num
        self.cuda = cuda
    def forward(self, vertices: "(bs, vertice_num, 3)",
                feature_map: "(bs, vertice_num, channel_num)", idx=None):
        """
        Return:
        ...
        # sample_idx = sample_idx.expand(bs, -1)
        # print(sample_idx.type())
        # (bs, pool_num, channel_num)

        else:
            vertices_pool = index_points(vertices, idx, cuda=self.cuda, is_group=False)
            torch.cuda.empty_cache()
            feature_map_pool = index_points(pooled_feature, idx, cuda=self.cuda, is_group=False)
            torch.cuda.empty_cache()
        # feature_map_pool = feature_map_pool
        return vertices_pool, feature_map_pool


class Encoder(nn.Module):
    def __init__(self, support_num: int = 10, pooling_rate: int = 2, neighbor_num: int = 8,  in_c: int = 3, out_c: int = 64, cuda=False):
        super().__init__()
        self.neighbor_num = neighbor_num
        self.pooling_rate = pooling_rate
        self.conv_0 = Operator3D(kernel_num=out_c, support_num=support_num)
        self.dropping = torch.nn.Dropout(p=0.1, inplace=True)
        self.conv_1 = OperatorND(out_c, in_c, support_num=support_num)
        self.pool_1 = Pooling(pooling_rate=self.pooling_rate, neighbor_num=neighbor_num, cuda=cuda)
        self.conv_2 = OperatorND(in_c, out_c, support_num=support_num)
        self.conv_3 = OperatorND(out_c, out_c, support_num=support_num)
        self.pool_2 = Pooling(pooling_rate=self.pooling_rate, neighbor_num=neighbor_num, cuda=cuda)
    ...
    def forward(self, vertices: "(bs, vertice_num, 3)", idx=None, feature=None):
        bs, vertice_num, _ = vertices.size()
        neighbor_index = get_neighbor_index(vertices, self.neighbor_num)
        if feature is None:
            ...
            # fm_5 = self.conv_1(neighbor_index, vertices, fm_1)
            # fm_5 = F.relu(fm_5, inplace=True)
            # vertices, feature = self.pool_1(vertices, fm_1, idx)
            # vertices_anchor = vertices
            feature = fm_1
        else:
            fm_2 = self.conv_2(neighbor_index, vertices, feature)
            fm_2 = F.relu(fm_2, inplace=True)
            fm_3 = self.conv_3(neighbor_index, vertices, fm_2)
            fm_3 = F.relu(fm_3, inplace=True)
            feature = fm_3
            # vertices, feature = self.pool_2(vertices, fm_3, idx)
        # neighbor_index = get_neighbor_index(vertices, self.neighbor_num)
        #
